...
struct frb_sloth_search_algorithm : public frb_search_algorithm_base
{
    ...
    //
    // weight_arr is a shape-(ndm,nups) array, containing the optimal weight
    // for the given DM channel and subsampling phase (the subsampling phase
    // is defined mod nups)
    //
    vector<double> weight_arr;

    frb_sloth_search_algorithm(double epsilon, int nupsample);
...
frb_sloth_search_algorithm::frb_sloth_search_algorithm(double epsilon_, int nupsample)
    : epsilon(epsilon_), nups(nupsample), ndm(0), max_save_n(0), nwork(0)
{
    xassert(epsilon > 0.0);
    xassert(nupsample >= 1);
    xassert(nupsample <= 16);      // surely unintentional
...
void frb_sloth_search_algorithm::search_init(const frb_search_params &p)
{
    ...
    //
    this->weight_arr.resize(ndm * nups);
    memset(&weight_arr[0], 0, ndm * nups * sizeof(weight_arr[0]));
	...
	for (int idm = 0; idm < ndm; idm++) {
	    ...
	    double dt0 = nups * dispersion_delay(dm,nu_hi)/ts;
	    double dt1 = nups * dispersion_delay(dm,nu_lo)/ts;
		...
		int j0 = ((i0+s)/nups) * nups;
		int j1 = ((i1+s)/nups) * nups;

		if (j0 == j1)
		    weight_arr[idm*nups + s] += 1.0;
		else {
		    double w = 0.0;
		    w += square((j0+nups) - (dt0+s));   // contribution from first wide sample
		    w += (j1-j0-nups) * nups;           // contribution from middle samples (note no square on nups)
		    w += square((dt1+s) - j1);          // contribution from last wide sample

		    weight_arr[idm*nups + s] += w / square(dt1-dt0);
		}
	    }
	}
    }

    for (int i = 0; i < ndm*nups; i++)
...
void frb_sloth_search_algorithm::search_chunk(const float *chunk, int ichunk, float *debug_buffer)
{
    ...
    int ncsum = ns*nups + 2*max_save_n + 2;   // the "+2" is to accommodate roundoff error
	...
	for (int i = 0; i < ns; i++)
	    for (int j = icsum + i*nups; j < icsum + (i+1)*nups; j++)
		cumsum[j+1] = cumsum[j] + chunk[ichan*ns + i];

	for (int j = icsum + ns*nups + 1; j < ncsum; j++)
	    cumsum[j] = cumsum[icsum + ns*nups];
    ...
    //
    for (int idm = 0; idm < ndm; idm++) {
	int n = nups * ns;      // number of finalized entries
	int m = save_n[idm];    // number of partially computed entries

	// Offset between work_buf and weight_arr (defined mod nups)
	int s = nups - ((-save_i0[idm]) % nups);
	...
	// Convert to upsampled units, relative to start of work buffer
	t0 = nups * (t0/ts - ichunk*ns) - save_i0[idm];
	t1 = nups * (t1/ts - ichunk*ns) - save_i0[idm];
	...
	for (int it = it0; it < it1; it++) {
	    float w = weight_arr[idm*nups + ((it+s) % nups)];
...
frb_search_algorithm_base *sloth(double epsilon, int nupsample)
{
    return new frb_sloth_search_algorithm(epsilon, nupsample);
}
